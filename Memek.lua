...
-- LOAD LIB
local repo = "https://raw.githubusercontent.com/Wisnuxyz/WisnuX/refs/heads/main/"
...
local Window = Library:CreateWindow({
    Title = "Wisnu Hub",
    Footer = 'Violence District v0.0.1 | by Wisnu Hub',
    Icon = "96848424314690",
...
local TeleportBox  = Tabs.ESP:AddRightGroupbox("Teleport", "map-pin")
...
-- Pasang atau update highlight
local function applyHighlight(adornee, color)
    -- Cari highlight yang sudah ada untuk objek ini
    for i, hl in pairs(HighlightObjects) do
        if hl.Adornee == adornee then
            hl.FillColor = color
    ...
    hl.FillColor = color
    hl.OutlineColor = color
    hl.FillTransparency = 0.4     -- Glow transparan
...
-- Hapus highlight dari objek
local function removeHighlight(adornee)
    for i, hl in pairs(HighlightObjects) do
        if hl.Adornee == adornee then
            hl:Destroy()
            HighlightObjects[i] = nil
            break
        end
    end
end

-- Update loop ESP (Hanya Highlight)
local function updateESP()
    clearESP()
    if not (ESP.Survivor or ESP.Killer or ESP.Generator or ESP.Pallet or ESP.Window or ESP.SCP) then return end
        ...
        if player ~= LocalPlayer and player.Character then
            ...
            if hum and hum.Health > 0 and root then
                ...
                if (ESP.Survivor and isSurvivor) then
                    applyHighlight(char, TeamColors.Survivor)
                elseif (ESP.Killer and isKiller) then
                    applyHighlight(char, TeamColors.Killer)
                else
                    removeHighlight(char)
                end
            end
        ...
        for gen in pairs(ESPCache.Generators) do
            if gen and gen.Parent then
                ...
                if part then
                    ...
                    if dist <= ESP.Distance then
                        applyHighlight(gen, GeneratorColor)
                    else
                        removeHighlight(gen)
                    end
                end
    ...
    -- ---- PALLET ----
    if ESP.Pallet then
        for pallet in pairs(ESPCache.Pallets) do
            if pallet and pallet.Parent then
                local part = pallet:FindFirstChildWhichIsA("BasePart")
                if part then
                    local dist = (part.Position - cam.CFrame.Position).Magnitude
                    if dist <= ESP.Distance then
                        applyHighlight(pallet, PalletColor)
                    else
                        removeHighlight(pallet)
                    end
                end
            end
        end
        ...
        for window in pairs(ESPCache.Windows) do
            if window and window.Parent then
                ...
                if part then
                    local dist = (part.Position - cam.CFrame.Position).Magnitude
                    if dist <= ESP.Distance then
                        applyHighlight(window, WindowColor)
                    else
                        removeHighlight(window)
                    end
                end
            end
        end
    ...
    -- ---- SCP ----
    if ESP.SCP then
        for scp in pairs(ESPCache.SCP) do
            if scp and scp.Parent then
                local part
                if scp:IsA("Model") then
                    part = scp:FindFirstChildWhichIsA("BasePart") or scp.PrimaryPart
                else
                    part = scp
                end
                if part then
...
RunService.RenderStepped:Connect(function()
    ...
    if now - Timers.lastESPUpdate >= 0.05 then
        ...
        -- Panggil updateESP highlight only
        pcall(updateESP)
        applyVisual()
...
-- ESP UI (Hanya highlight, tanpa status)
...
-- PLAYER UI
-- Tambahkan ini di bagian AbilityTab (sekitar baris 1000-1100)
...
ParryBox:AddCheckbox("Surv_ParrySafety", { Text = "Safety Parry", Default = false, Callback = function(Value) Config.Surv_ParrySafety = Value end })
...
ParryBox:AddDropdown("FakeParryAnim", {
    Text = "Animation", Values = {"Enten","Stopwatch","Fih","BloodShield"}, Default = 1,
...
AimlockBox:AddDropdown("GunAimPart", {
    Text = "Aim Part", Values = {"Head","HumanoidRootPart","Torso", "Root"}, Default = 2,
...
ToFBox:AddDropdown("ToFAimPart", {
    Values = {"HumanoidRootPart", "Head", "Torso", "Root"}, Default = 1, Text = "Aim Part",
...
-- ==========================================
-- SCRIPT MOONWALK EKSTERNAL V3 (VD_T MOONWALK) - SEMPURNA
-- ==========================================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
...
SettingBox:AddToggle("HeaderGlowToggle", {
    Text = "Glow AccentBar",
    Default = true,
    Callback = function(Value)
...
SettingBox:AddToggle("ShowProfile", {
    Text = "Show Profile",
    Default = true,
    Callback = function(Value)
...
ThemeManager:SetFolder("Wisnu")
SaveManager:SetFolder("Wisnu/configs")
SaveManager:BuildConfigSection(Tabs["UISettings"])
ThemeManager:ApplyToTab(Tabs["UISettings"])
SaveManager:LoadAutoloadConfig()

-- ==================== NOTIFICATION ====================
Library:Notify({ Title = "Wisnu Hub", Description = "Berhasil Dimuat!", Time = 3 })
end
